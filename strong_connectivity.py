...
#------------------------------------------------------------------------------
# strong connectivity Test, using simplified Tarjan

def __is_strong(G, x, pref, cpt):
    """
    DFS of G from x
    pref the prefix vector and cpt its counter
    return (return_x, cpt) where return_x s the return value of x
    """
    cpt += 1
    pref[x] = cpt
    return_x = pref[x]
    ...
    for y in G.adjlists[x]:
        if pref[y] == 0:
            (return_y, cpt, ok) = __is_strong(G, y, pref, cpt)
            if not ok:
                return (return_x, cpt, False)
            return_x = min(return_x, return_y)
        else:
            return_x = min(return_x, pref[y])
    ...
    return (return_x, cpt, True)

def is_strong(G, x):
    pref = [0] * G.order
    cpt = 0
    (return_x, cpt, ok) = __is_strong(G, x, pref, cpt)
    return ok and cpt == G.order
